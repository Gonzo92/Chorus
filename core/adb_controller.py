# ============================================================
#  Chorus v2.4  –  core/adb_controller.py
# ============================================================
...
def install_apk(serial: str, apk_path: str, replace: bool = False) -> tuple[bool, str]:
    """
    Install an APK on a device via ADB.

    Args:
        serial: Device serial number
        apk_path: Local path to the APK file
        replace: If True, use -r flag to replace existing app
    ...
    rc, stdout, stderr = adb(serial, "install", flag, apk_path, timeout=60)
    if rc == 0:
        if "Success" in stdout or "Success" in stderr:
            return True, "Installed successfully"
        return True, stdout.strip() or stderr.strip() or "Installed"
    else:
        msg = stderr.strip() or stdout.strip() or f"ADB error (rc={rc})"
        return False, msg
