# ============================================================
#  Chorus v2.5  –  gui/pair_tab.py
#  Pair status panels: MO/MT serials, cycle counter, progress,
#  stage display, timer, pass/fail counts, scrcpy buttons.
# ============================================================
...
from utils.theme import BG, BG2, BG3, FG, FG_DIM, BLUE, YELLOW, GREEN, RED, CYAN, MONO, SANS, STAGE_COLOR
...
class PairTab:
    ...
    def _on_scrcpy(self, pair, role):
        """Notify main.py that scrcpy was requested."""
        self._on_scrcpy(pair, role)

    # ── public update methods ──────────────────────────────────
    def set_enabled(self, pair, enabled):
        """Dim/brighten panel when pair is enabled/disabled."""
        w = self._panels[pair]
    ...
    def update(self, pair, state, loops):
        """Update panel with current test state."""
        s = state[pair]
        w = self._panels[pair]
        color = BLUE if pair == "dut" else YELLOW
