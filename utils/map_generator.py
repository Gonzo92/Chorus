...
def generate_html(csv_path: str, output_path: str, test_name: str) -> str:
    ...
    ">
        <b style="font-size:13px">Chorus v2.4 GPS</b><br>
    ...
    <div style="
        ...
        box-shadow: 2px 2px 6px rgba(0,0,0,0.3);
    ">
        Chorus v2.4 — {test_name} — {datetime.now().strftime("%Y-%m-%d %H:%M")}
    </div>
    """
...
def generate_png(csv_path: str, output_path: str) -> str:
    ...
    ax.set_title(f"Chorus v2.4 — {data[0].get('pair', 'test').upper()} GPS Tracking", fontsize=13, fontweight="bold")
    ax.grid(True, alpha=0.3, linestyle="-")
    ax.legend(loc="upper right", fontsize=9, framealpha=0.9)
    
    # Add version and timestamp
    fig.text(0.98, 0.02, f"Chorus v2.4 | {datetime.now().strftime('%Y-%m-%d %H:%M')}",
