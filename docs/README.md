...
├── adb_controller.py    ← ADB commands (call, answer, signal info, scrcpy)
├── call_monitor.py      ← call cycle orchestration with per-second callbacks
├── report.py            ← CSV logging and summary report generation
├── device_picker.py     ← ADB device scanning and role assignment dialog
├── rat_controller.py    ← RAT (Radio Access Technology) setting functions
